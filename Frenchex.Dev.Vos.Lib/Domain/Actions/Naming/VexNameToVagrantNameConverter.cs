...
namespace Frenchex.Dev.Vos.Lib.Domain.Actions.Naming
{
    ...
    {
        string[] ConvertAll(string[] names, string workingDirectory, Bases.Configuration configuration);
    }

    public class VexNameToVagrantNameConverter : IVexNameToVagrantNameConverter
    {
        private static readonly Regex patternFromToWildcard = new("(?<machine>.*)\\-\\[(?<instance>.*)\\]", RegexOptions.Compiled);

        private static readonly Regex patternFromTo = new("(?<machine>.*)\\-(?<instance>.*)", RegexOptions.Compiled);

        public string[] ConvertAll(string[] inputNamedPatterns, string workingDirectory, Bases.Configuration configuration)
        {
            ...
            foreach (var name in inputNamedPatterns)
            {
                ...
                switch (matchesPatternFromToWildcard.Count)
                {
                    case 0:
                    {
                        ...
                        var instanceAsStr = string.Format(configuration?.Vagrant?.NumberingFormat ?? "{0,2:D2}", instances == "0" ? 0 : int.Parse(instances));
