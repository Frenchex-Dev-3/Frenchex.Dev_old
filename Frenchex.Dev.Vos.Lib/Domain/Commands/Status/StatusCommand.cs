...
namespace Frenchex.Dev.Vos.Lib.Domain.Commands.Status
{
    public class StatusCommand : RootCommand, IStatusCommand
    {
        ...
        private readonly Vagrant.Lib.Domain.Commands.Status.IStatusCommandRequestBuilderFactory _vagrantStatusCommandRequestBuilderFactory;
        ...
        public async Task<IStatusCommandResponse> Execute(IStatusCommandRequest request)
        {
            ...
            var statuses = new Dictionary<string, VagrantMachineStatusEnum>();

            foreach (var item in statusesOutput)
            {
                var statusLineSplit = item.Split(" ").ToList();
                var machine = statusLineSplit.First().Trim();
                var statusString = (statusLineSplit[^3] + " " + statusLineSplit[^2].Trim()).Trim();
                var providerString = statusLineSplit[^1].Replace("(", "").Replace(")", "").Trim();

                VagrantMachineStatusEnum status = VagrantMachineStatusEnum.NOT_CREATED;

                switch (statusString)
                {
                    case "not created":
                        status = VagrantMachineStatusEnum.NOT_CREATED;
                        break;
                    case "running":
