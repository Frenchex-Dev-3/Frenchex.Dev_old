...
namespace Frenchex.Dev.Vos.Lib.Domain.Commands.Ssh
{
    public class SshCommand : RootCommand, ISshCommand
    {
        ...
        private readonly Vagrant.Lib.Domain.Commands.Ssh.ISshCommandRequestBuilderFactory _vagrantSshCommandRequestBuilderFactory;
        public SshCommand(
        ...
        public async Task<ISshCommandResponse> Execute(ISshCommandRequest request)
        {
            ...
            var output = Encoding.UTF8.GetString(response?.ProcessExecutionResult?.OutputStream?.ToArray() ?? Array.Empty<byte>());
